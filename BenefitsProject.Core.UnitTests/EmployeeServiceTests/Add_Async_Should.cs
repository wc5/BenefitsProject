...
namespace BenefitsProject.Core.UnitTests.EmployeeServiceTests
{
    public class Add_Async_Should
    {
        ...
        public Add_Async_Should()
        {
            _fakeRepository = new Mock<IEmployeeRepository>();
            _fakeRepository
                .Setup(r => r.Add_Async(It.IsAny<Employee>()))
        ...
        public async Task InvokeOnceBenefitCalculatorCalculateAndSetSalaryAndBenefitData()
        {
            ...
            var employee = await _employeeService.Add_Async(newEmployee);

            _fakeCalculator.Verify(c => c.CalculateAndSetSalaryAndBenefitData(newEmployee), Times.Once);
        }

        [Fact]
        public async Task InvokeOnceEmployeeRepositoryAdd_Async()
        {
            var newEmployee = new Employee();

            var employee = await _employeeService.Add_Async(newEmployee);

            _fakeRepository.Verify(r => r.Add_Async(newEmployee), Times.Once);
        }

        [Fact]
        public async Task ReturnExpectedEmployee()
        {
            var employee = await _employeeService.Add_Async(new Employee());
